$(document).ready(function () {
  ...
  function resetFormRecovery() {
    $("#idRecovery").val("").prop("readonly", false).show();
    $("label[for='idRecovery']").show();
    $("#idUserData").val("").prop("readonly", false);
    $("#recoveryKey").val("").prop("readonly", false);
    $("#expirationDate").val("").prop("readonly", false);
  ...
  $(document).on("click", "#save-btn-recovery", function (event) {
    ...
    if (ID != null) {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          console.error(xhr.responseText);
          alert("There was an error trying to update the recovery.");
        },
      });
    } else {
      var formData = {
